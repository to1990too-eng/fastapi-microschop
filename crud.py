...
async def get_user_by_username(session: AsyncSession, username: str) -> User | None:
    ...
    # user: User | None = await session.scalars(stmt)
...
async def main_relations(session: AsyncSession):
    async with db_helper.session_factory() as session:
        ...
        # await get_user_by_username(session=session, username="njk")
        # await demo_m2m()
...
async def create_orders_and_products(session: AsyncSession):
    ...
    mouse = await create_product(
        session,
        name="Mouse",
        description="Create gaming mouse",
        price=123,
    )
    ...
    keyboard = await create_product(
        session,
        name="keyboard",
        description="Create gaming keybord",
        price=157,
    )
...
async def demo_get_orders_with_products_secondary(session: AsyncSession):
